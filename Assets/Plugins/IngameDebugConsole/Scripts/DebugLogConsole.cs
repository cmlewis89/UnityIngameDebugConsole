...
public class ConsoleButtonInfo
{
	public readonly MethodInfo method;
	public readonly string buttonTitle;

	public ConsoleButtonInfo( MethodInfo method, string title )
	{
		this.method = method;
...
public static class DebugLogConsole
{
	...
	// Console Button list
	public static Dictionary<string, List<ConsoleButtonInfo>> consoleButtons = new Dictionary<string, List<ConsoleButtonInfo>>();
		...
		foreach( var assembly in assemblies )
		{
			foreach( var type in assembly.GetExportedTypes() )
			{
				foreach( var method in type.GetMethods( BindingFlags.Static | BindingFlags.Public | BindingFlags.DeclaredOnly ) )
				{
					...
					//fetch console button methods
					foreach( var attribute in method.GetCustomAttributes( typeof( ConsoleButtonAttribute ), false ) )
					{
						ConsoleButtonAttribute buttonMethod = attribute as ConsoleButtonAttribute;
						if( buttonMethod != null ) {
							AddButton(buttonMethod.ButtonTitle, method, buttonMethod.ButtonGroup);
						}
					}
				}
			}
	...
	// Add a debug button with this method
	public static void AddButton (string buttonTitle, MethodInfo method, string buttonGroup = null)
	{
		//buttons without groups go to root
		if (string.IsNullOrEmpty(buttonGroup)) {
			buttonGroup = "root";
		}
		//add a grouping if it doesnt exist yet
		if (!consoleButtons.ContainsKey(buttonGroup)) {
			consoleButtons.Add(buttonGroup, new List<ConsoleButtonInfo>());
		}
		consoleButtons[buttonGroup].Add(new ConsoleButtonInfo(method, buttonTitle));
	}
